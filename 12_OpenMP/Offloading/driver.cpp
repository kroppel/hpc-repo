...
#include <cmath>
#include <chrono>
#include "kernels/triad_offloading.h"

/**
 * Benchmarks triad_offloading.
 *
...
int main( int i_argc, char const * i_argv[] ) {
  ...
  if( i_argc != 5 ) {
    std::cerr << "use as: ./build/triad_offloading N_VALUES N_REPEATS N_THREADS N_TEAMS" << std::endl;
    return EXIT_FAILURE;
  }
  uint64_t l_nValues = atoll( i_argv[1] );
  uint64_t l_nRepeats = atoll( i_argv[2] );
  int l_nTeams = atoll( i_argv[3] );
  int l_nThreads = atoll( i_argv[4] );
  ...
  std::cout << "  N_TEAMS: " << l_nTeams << std::endl;
  std::cout << "  N_THREADS: " << l_nThreads << std::endl;

  std::cout << "benchmarking triad_offloading" << std::endl;
  bench( l_nValues,
         l_nRepeats,
         0,
         l_nTeams,
